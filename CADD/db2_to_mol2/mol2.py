...
def read_Mol2_file(file):
    ...
    for line in lines:
         ...
         if ((len(linesplit) >= 9 )and (flag_atom)):
             ...
             temp_atom = atom(X,Y,Z,Q,atom_type,atom_name,atom_num,res_num,res_name)
             atom_list.append(temp_atom)
             if res_num in residue_list: residue_list[res_num].append(temp_atom)
             else: residue_list[res_num] = [temp_atom]
...
def write_mol2(molecule,filename):
    ...
    outmol2.write("%-5d %-5d %-5d 0     0\n" % (len(molecule.atom_list), len(molecule.bond_list), len(molecule.residue_list.keys()))) 
    ...
    for resnum in molecule.residue_list.keys():
        ...
        outmol2.write("%-3d %-5s %-5d RESIDUE    1   A     %-5s 1\n" % (resid_dic[resnum], molecule.residue_list[resnum][0].resname, atom_dic[molecule.residue_list[resnum][0].num], molecule.residue_list[resnum][0].resname[0:3]))
        count = count + 1
    outmol2.close()
    return
#################################################################################################################
